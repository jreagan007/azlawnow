/**
 * AZ Law Now editorial chart, arizona-schools-merv-13-filter-bypass.
 * 2-bar PM2.5 air-quality standard comparison: EPA 9 µg/m³ vs WHO 5 µg/m³.
 * Schools tout MERV 13 but ASHRAE 241 says bypass air = lower MERV.
 */
import sharp from 'sharp';
import { mkdirSync } from 'fs';
import { TOKENS, FONTS, SIZES, PATHS } from './lib/azlawnow-chart-tokens.js';

const SLUG = 'arizona-schools-merv-13-filter-bypass';
const CAPTION_SLUG = 'pm2-5-air-quality-standards-epa-vs-who-2024';
const PUBLISH = 'May 6, 2026';
const BYLINE = 'Brendan Franks';
const OUTDIR = '/Users/taqticlaw/Projects/azlawnow/public/embeds';

type Bar = { label: string; tag: string; value: number; unit: string; isHighlight: boolean };
const BARS: Bar[] = [
  { label: 'EPA standard, U.S.',         tag: 'TIGHTENED FEB 2024',           value: 9, unit: 'µg/m³', isHighlight: false },
  { label: 'WHO guideline, global',      tag: 'PUBLISHED 2021',               value: 5, unit: 'µg/m³', isHighlight: true  },
];
const X_MAX = 12;
...
function buildSvg(layout: Layout): string {
  ...
  const tickSvg = ticks.map(t => { const x = xScale(t); return `<line x1="${x}" y1="${CHART_TOP - 6}" x2="${x}" y2="${CHART_TOP - 1}" stroke="${TOKENS.hairline}" stroke-width="1"/><text x="${x}" y="${CHART_TOP - 12}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" fill="${TOKENS.slateLight}" text-anchor="middle">${t}</text>`; }).join('');
  ...
  const axisLabelSvg = `<text x="${CHART_LEFT + CHART_W / 2}" y="${CHART_BOTTOM + 32}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" letter-spacing="2" fill="${TOKENS.slate}" text-anchor="middle">PM2.5 ANNUAL EXPOSURE LIMIT BY GOVERNING BODY</text>`;
  const headlineSvg = `
    <text x="${SAFE}" y="${layout.EYEBROW_Y}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" letter-spacing="3" font-weight="700" fill="${TOKENS.vermillion}">AZ LAW NOW · INDOOR AIR QUALITY INVESTIGATION</text>
    <text x="${SAFE}" y="${layout.HEADLINE_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">EPA tightened the PM2.5 standard to 9 µg/m³.</text>
    <text x="${SAFE}" y="${layout.HEADLINE_2_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">The WHO has been at 5 since 2021.</text>
    ${layout.showSubdeck && layout.SUBDECK_Y ? `<text x="${SAFE}" y="${layout.SUBDECK_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyL}" font-style="italic" fill="${TOKENS.slate}">Maricopa County is in serious non-attainment for PM10. Schools tout MERV 13. ASHRAE 241 says bypass air equals lower MERV.</text>` : ''}
  `;
  const captionSvg = `
    ...
    <text x="${SAFE}" y="${layout.SOURCE_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyS}" font-style="italic" fill="${TOKENS.slate}">Source: EPA NAAQS revision (Feb 2024); WHO global air quality guidelines (2021); ASHRAE 52.2-2017, 241-2023.</text>
...
async function render(name: string, layout: Layout): Promise<string> {
  ...
  const out = `${OUTDIR}/merv-13-${name}.png`;
...
const master: Layout = { W: 1200, H: 675, SAFE: 80, EYEBROW_Y: 64, HEADLINE_Y: 110, HEADLINE_2_Y: 150, SUBDECK_Y: 188, CHART_TOP: 240, CHART_BOTTOM: 510, CHART_LEFT: 360, CHART_RIGHT: 1110, BAR_H: 80, CAPTION_Y: 565, SOURCE_Y: 590, BYLINE_Y: 615, LOGO_W: 130, LOGO_RIGHT: 80, LOGO_BOTTOM: 28, HEADLINE_SIZE: 28, numeralSize: 32, labelSize: 18, showSubdeck: true };
const igSquare: Layout = { W: 1080, H: 1080, SAFE: 80, EYEBROW_Y: 90, HEADLINE_Y: 150, HEADLINE_2_Y: 198, SUBDECK_Y: 250, CHART_TOP: 360, CHART_BOTTOM: 800, CHART_LEFT: 360, CHART_RIGHT: 1000, BAR_H: 130, CAPTION_Y: 880, SOURCE_Y: 915, BYLINE_Y: 950, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 50, HEADLINE_SIZE: 32, numeralSize: 38, labelSize: 22, showSubdeck: true };
const igPortrait: Layout = { W: 1080, H: 1350, SAFE: 80, EYEBROW_Y: 100, HEADLINE_Y: 170, HEADLINE_2_Y: 218, SUBDECK_Y: 270, CHART_TOP: 410, CHART_BOTTOM: 1000, CHART_LEFT: 360, CHART_RIGHT: 1000, BAR_H: 170, CAPTION_Y: 1115, SOURCE_Y: 1155, BYLINE_Y: 1195, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 70, HEADLINE_SIZE: 34, numeralSize: 42, labelSize: 22, showSubdeck: true };
const fbFeed: Layout = { W: 1200, H: 630, SAFE: 70, EYEBROW_Y: 60, HEADLINE_Y: 104, HEADLINE_2_Y: 144, SUBDECK_Y: 178, CHART_TOP: 222, CHART_BOTTOM: 478, CHART_LEFT: 360, CHART_RIGHT: 1130, BAR_H: 78, CAPTION_Y: 522, SOURCE_Y: 548, BYLINE_Y: 572, LOGO_W: 130, LOGO_RIGHT: 70, LOGO_BOTTOM: 24, HEADLINE_SIZE: 28, numeralSize: 32, labelSize: 17, showSubdeck: true };
...
await sharp(`${OUTDIR}/merv-13-ig-portrait-1080x1350.png`).jpeg({ quality: 90 }).toFile(`${igOut}/ig-merv-13-chart-2026-05.jpg`);
await sharp(`${OUTDIR}/merv-13-fb-feed-1200x630.png`).png({ compressionLevel: 9 }).toFile(`${fbOut}/fb-merv-13-chart-2026-05.png`);
console.log(`IG: ${igOut}/ig-merv-13-chart-2026-05.jpg`);
console.log(`FB: ${fbOut}/fb-merv-13-chart-2026-05.png`);

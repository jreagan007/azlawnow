...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from ambient light sources only. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no faces, no hands, no people visible, no children, no brand logos, no Walgreens signage, no pharmacy logos.`;

const PROMPT = `Wide-angle documentary photograph of a generic suburban Phoenix Arizona drive-through pharmacy at dusk. A standalone single-story stucco building with a flat roof and a clean modern canopy over an empty drive-through window lane. The pharmacy parking lot is mostly empty, with a single overhead lamp casting a warm pool of light on the asphalt. Through the unbranded plate-glass storefront windows, internal fluorescent lighting glows pale white. A row of distant strip-mall signage stands silhouetted against a dusty desert sunset sky in the far background, no readable text. A row of low desert landscaping with palms and gravel. Atmosphere: routine commercial dispensing infrastructure that handles federal records nobody sees. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-walgreens-350m-arizona-opioid-stores.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'walgreens-350m-arizona-opioid-stores';
const HEADLINE = '$350M Settlement. Which AZ Stores?';

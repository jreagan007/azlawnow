/**
 * AZ Law Now editorial chart, arizona-daycare-violations.
 * 4-city horizontal bar of repeat-violation facilities (3+ findings) by city.
 * Surprise highlighted as the leader.
 */
import sharp from 'sharp';
import { mkdirSync } from 'fs';
import { TOKENS, FONTS, SIZES, PATHS } from './lib/azlawnow-chart-tokens.js';

const SLUG = 'arizona-daycare-violations';
const CAPTION_SLUG = 'maricopa-west-valley-daycare-repeat-violation-facilities-by-city-2023-2025';
...
const BARS: Bar[] = [
  { label: 'Surprise',  tag: '67 LICENSED',  value: 17, isHighlight: true  },
  { label: 'Goodyear',  tag: '54 LICENSED',  value: 14, isHighlight: false },
  { label: 'Avondale',  tag: '41 LICENSED',  value: 11, isHighlight: false },
  { label: 'Buckeye',   tag: '38 LICENSED',  value: 9,  isHighlight: false },
];
const X_MAX = 20;
...
interface Layout {
  ...
  CHART_TOP: number; CHART_BOTTOM: number; CHART_LEFT: number; CHART_RIGHT: number;
  BAR_H: number; CAPTION_Y: number; SOURCE_Y: number; BYLINE_Y: number;
  LOGO_W: number; LOGO_RIGHT: number; LOGO_BOTTOM: number;
  HEADLINE_SIZE: number; numeralSize: number; labelSize: number; showSubdeck: boolean;
}
...
function buildSvg(layout: Layout): string {
  ...
  `;
  const axisLabelSvg = `
    <text x="${CHART_LEFT + CHART_W / 2}" y="${CHART_BOTTOM + 32}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" letter-spacing="2" fill="${TOKENS.slate}" text-anchor="middle">FACILITIES CITED 3+ TIMES, 2023-2025</text>
  `;
  const headlineSvg = `
    <text x="${SAFE}" y="${layout.EYEBROW_Y}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" letter-spacing="3" font-weight="700" fill="${TOKENS.vermillion}">AZ LAW NOW · CHILD SAFETY INVESTIGATION</text>
    <text x="${SAFE}" y="${layout.HEADLINE_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">51 daycare facilities cited 3 or more times</text>
    <text x="${SAFE}" y="${layout.HEADLINE_2_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">across 4 West Valley cities, 2023-2025.</text>
    ${layout.showSubdeck && layout.SUBDECK_Y ? `
      <text x="${SAFE}" y="${layout.SUBDECK_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyL}" font-style="italic" fill="${TOKENS.slate}">Staff-ratio violations are 40% of West Valley citations. Repeat violators can operate for years.</text>
    ` : ''}
  `;
  const captionSvg = `
    ...
    <text x="${SAFE}" y="${layout.SOURCE_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyS}" font-style="italic" fill="${TOKENS.slate}">Source: AZ DHS CareCheck inspection records, public records requests for unposted reports.</text>
...
async function render(name: string, layout: Layout): Promise<string> {
  ...
  const out = `${OUTDIR}/daycare-violations-${name}.png`;
...
const master: Layout = { W: 1200, H: 675, SAFE: 80, EYEBROW_Y: 64, HEADLINE_Y: 110, HEADLINE_2_Y: 150, SUBDECK_Y: 188, CHART_TOP: 240, CHART_BOTTOM: 510, CHART_LEFT: 280, CHART_RIGHT: 1110, BAR_H: 48, CAPTION_Y: 565, SOURCE_Y: 590, BYLINE_Y: 615, LOGO_W: 130, LOGO_RIGHT: 80, LOGO_BOTTOM: 28, HEADLINE_SIZE: 28, numeralSize: 28, labelSize: 18, showSubdeck: true };
const igSquare: Layout = { W: 1080, H: 1080, SAFE: 80, EYEBROW_Y: 90, HEADLINE_Y: 150, HEADLINE_2_Y: 198, SUBDECK_Y: 248, CHART_TOP: 360, CHART_BOTTOM: 800, CHART_LEFT: 290, CHART_RIGHT: 1000, BAR_H: 70, CAPTION_Y: 880, SOURCE_Y: 915, BYLINE_Y: 950, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 50, HEADLINE_SIZE: 34, numeralSize: 34, labelSize: 22, showSubdeck: true };
const igPortrait: Layout = { W: 1080, H: 1350, SAFE: 80, EYEBROW_Y: 100, HEADLINE_Y: 170, HEADLINE_2_Y: 218, SUBDECK_Y: 270, CHART_TOP: 410, CHART_BOTTOM: 1000, CHART_LEFT: 290, CHART_RIGHT: 1000, BAR_H: 90, CAPTION_Y: 1115, SOURCE_Y: 1155, BYLINE_Y: 1195, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 70, HEADLINE_SIZE: 36, numeralSize: 38, labelSize: 22, showSubdeck: true };
const fbFeed: Layout = { W: 1200, H: 630, SAFE: 70, EYEBROW_Y: 60, HEADLINE_Y: 104, HEADLINE_2_Y: 144, SUBDECK_Y: 178, CHART_TOP: 222, CHART_BOTTOM: 478, CHART_LEFT: 280, CHART_RIGHT: 1130, BAR_H: 46, CAPTION_Y: 522, SOURCE_Y: 548, BYLINE_Y: 572, LOGO_W: 130, LOGO_RIGHT: 70, LOGO_BOTTOM: 24, HEADLINE_SIZE: 28, numeralSize: 28, labelSize: 17, showSubdeck: true };
...
await sharp(`${OUTDIR}/daycare-violations-ig-portrait-1080x1350.png`).jpeg({ quality: 90 }).toFile(`${igOut}/ig-daycare-violations-chart-2026-05.jpg`);
await sharp(`${OUTDIR}/daycare-violations-fb-feed-1200x630.png`).png({ compressionLevel: 9 }).toFile(`${fbOut}/fb-daycare-violations-chart-2026-05.png`);
console.log(`IG: ${igOut}/ig-daycare-violations-chart-2026-05.jpg`);
console.log(`FB: ${fbOut}/fb-daycare-violations-chart-2026-05.png`);

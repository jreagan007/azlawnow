...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, soft warm gold accent from late-afternoon window light. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no faces, no hands, no people visible.`;

const PROMPT = `Wide-angle documentary photograph of an empty Arizona nursing home corridor late afternoon, soft golden light coming through a window at the end of the hallway. Linoleum floor with subtle scuff marks. A wheelchair sits empty against a wall next to a door with a privacy curtain partially drawn. Handrails run along both walls. A medication cart with locked drawers stands in the foreground. Fluorescent ceiling lights are on but not bright. The hallway extends into shadow. Atmosphere: an institutional space where the people living here depend on staff who may or may not have been there, where the ledger entry says care was delivered but the chair is empty. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-arizona-nursing-homes-billing-fraud-and-abuse.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'arizona-nursing-homes-billing-fraud-and-abuse';
const HEADLINE = 'AZ Nursing Homes: Billing Audit Is Care Audit';

...
const PROMPT = `Wide-angle documentary photograph of a single-story Sun City Arizona retirement-community home exterior at late-afternoon golden hour, 102-degree heat haze visible in the distance. Beige stucco walls with a low-pitched tile roof, manicured desert landscaping with lava rock and a saguaro silhouette. A residential electrical meter and gray utility box are visible on the side wall, slightly dust-covered. The driveway is empty. Light reflects off closed window blinds. The street is silent and empty in the harsh afternoon light. Saguaro shadows fall long across the asphalt. Atmosphere: a quiet residential neighborhood where utility decisions made in distant offices reach into homes nobody is checking on. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-aps-korman-heat-disconnect-7m-settlement.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'aps-korman-heat-disconnect-7m-settlement';
const HEADLINE = '$7M Settlement. 99-Degree Disconnect.';

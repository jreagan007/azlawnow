// Generate hero + branded OG for the Tempe / ASU pavement 180-day claim clock investigation.
...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from ambient light sources only. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no people faces, no hands, no license plates, no university insignia.`;

const PROMPT = `Wide-angle documentary photograph of a Tempe Arizona urban arterial street at dusk near a university campus district. Weathered asphalt with visible pavement cracks, a shallow pothole, faded lane striping, and a painted bicycle lane in poor condition. A parked electric scooter leans against a concrete curb in the foreground, its form dark against the warm streetlight glow. Palm silhouettes and low-rise brick buildings recede into the background. A lit streetlight casts a single warm gold pool onto the textured pavement. Empty street, no riders, no pedestrians, no vehicles in motion. Atmosphere: civic neglect visible in the surface, the quiet right before a commute. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-asu-tempe-pavement-claim-clock.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ✓ ${HERO_OUT}`);

// Branded OG composite: gradient overlay + vermillion accent + Georgia serif headline + logo.
const SLUG = 'tempe-asu-pavement-180-day-claim-clock';
const HEADLINE = '180 Days. Failing Asphalt. No Door to Serve.';

...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from ambient sunlight. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no faces, no hands, no people visible, no children.`;

const PROMPT = `Wide-angle documentary photograph of a quiet government records office interior at late afternoon, with rows of legacy beige metal filing cabinets stretching down a long fluorescent-lit hallway. Stacks of unsorted manila folders sit on a wood desk in the foreground, slightly open showing illegible blurred document edges. A single empty office chair faces an old beige CRT-style monitor displaying a generic blue spreadsheet pattern with no readable text. Sunlight slants through high vertical blinds casting long parallel shadows across the linoleum floor. Atmosphere: institutional records bureaucracy where insider access decisions get made far from public oversight. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-esa-fake-children-600k-grand-jury-indictment.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'esa-fake-children-600k-grand-jury-indictment';
const HEADLINE = '$600K ESA Fraud. 17 Children. 5 Fake.';

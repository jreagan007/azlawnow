/**
 * AZ Law Now editorial chart, arizona-school-bus-seat-belts.
 * 2-bar comparison: 8 states require seat belts on new school buses, 42 don't (AZ in the 42).
 */
import sharp from 'sharp';
import { mkdirSync } from 'fs';
import { TOKENS, FONTS, SIZES, PATHS } from './lib/azlawnow-chart-tokens.js';

const SLUG = 'arizona-school-bus-seat-belts';
const CAPTION_SLUG = 'us-states-requiring-seat-belts-on-new-school-buses-2025';
...
const BARS: Bar[] = [
  { label: 'Don\'t require seat belts',  tag: '42 STATES, INCLUDING ARIZONA', value: 42, isHighlight: true  },
  { label: 'Require seat belts',          tag: '8 STATES',                     value: 8,  isHighlight: false },
];
const X_MAX = 50;
...
function buildSvg(layout: Layout): string {
  ...
  const tickSvg = ticks.map(t => {
    const x = xScale(t);
    return `<line x1="${x}" y1="${CHART_TOP - 6}" x2="${x}" y2="${CHART_TOP - 1}" stroke="${TOKENS.hairline}" stroke-width="1"/><text x="${x}" y="${CHART_TOP - 12}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" fill="${TOKENS.slateLight}" text-anchor="middle">${t}</text>`;
  }).join('');
  ...
  const axisLabelSvg = `<text x="${CHART_LEFT + CHART_W / 2}" y="${CHART_BOTTOM + 32}" font-family="${FONTS.mono}" font-size="${SIZES.micro}" letter-spacing="2" fill="${TOKENS.slate}" text-anchor="middle">U.S. STATES BY SCHOOL BUS SEAT BELT REQUIREMENT, 2025</text>`;
  const headlineSvg = `
    ...
    <text x="${SAFE}" y="${layout.HEADLINE_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">8 states require seat belts on new school buses.</text>
    <text x="${SAFE}" y="${layout.HEADLINE_2_Y}" font-family="${FONTS.serif}" font-size="${HEADLINE_SIZE}" font-weight="700" fill="${TOKENS.ink}">Arizona is one of 42 that don't.</text>
    ${layout.showSubdeck && layout.SUBDECK_Y ? `<text x="${SAFE}" y="${layout.SUBDECK_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyL}" font-style="italic" fill="${TOKENS.slate}">971 school-transportation-related fatal crashes nationally, 2014 to 2023 (NHTSA FARS).</text>` : ''}
  `;
  const captionSvg = `
    ...
    <text x="${SAFE}" y="${layout.SOURCE_Y}" font-family="${FONTS.serif}" font-size="${SIZES.bodyS}" font-style="italic" fill="${TOKENS.slate}">Source: NHTSA School Bus Safety, FARS data 2014-2023, plus state-by-state legislative review.</text>
...
async function render(name: string, layout: Layout): Promise<string> {
  ...
  const out = `${OUTDIR}/school-bus-seat-belts-${name}.png`;
...
const master: Layout = { W: 1200, H: 675, SAFE: 80, EYEBROW_Y: 64, HEADLINE_Y: 110, HEADLINE_2_Y: 150, SUBDECK_Y: 188, CHART_TOP: 240, CHART_BOTTOM: 510, CHART_LEFT: 380, CHART_RIGHT: 1110, BAR_H: 80, CAPTION_Y: 565, SOURCE_Y: 590, BYLINE_Y: 615, LOGO_W: 130, LOGO_RIGHT: 80, LOGO_BOTTOM: 28, HEADLINE_SIZE: 28, numeralSize: 36, labelSize: 18, showSubdeck: true };
const igSquare: Layout = { W: 1080, H: 1080, SAFE: 80, EYEBROW_Y: 90, HEADLINE_Y: 150, HEADLINE_2_Y: 198, SUBDECK_Y: 250, CHART_TOP: 360, CHART_BOTTOM: 800, CHART_LEFT: 380, CHART_RIGHT: 1000, BAR_H: 130, CAPTION_Y: 880, SOURCE_Y: 915, BYLINE_Y: 950, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 50, HEADLINE_SIZE: 34, numeralSize: 44, labelSize: 22, showSubdeck: true };
const igPortrait: Layout = { W: 1080, H: 1350, SAFE: 80, EYEBROW_Y: 100, HEADLINE_Y: 170, HEADLINE_2_Y: 218, SUBDECK_Y: 270, CHART_TOP: 410, CHART_BOTTOM: 1000, CHART_LEFT: 380, CHART_RIGHT: 1000, BAR_H: 170, CAPTION_Y: 1115, SOURCE_Y: 1155, BYLINE_Y: 1195, LOGO_W: 160, LOGO_RIGHT: 80, LOGO_BOTTOM: 70, HEADLINE_SIZE: 36, numeralSize: 48, labelSize: 22, showSubdeck: true };
const fbFeed: Layout = { W: 1200, H: 630, SAFE: 70, EYEBROW_Y: 60, HEADLINE_Y: 104, HEADLINE_2_Y: 144, SUBDECK_Y: 178, CHART_TOP: 222, CHART_BOTTOM: 478, CHART_LEFT: 380, CHART_RIGHT: 1130, BAR_H: 78, CAPTION_Y: 522, SOURCE_Y: 548, BYLINE_Y: 572, LOGO_W: 130, LOGO_RIGHT: 70, LOGO_BOTTOM: 24, HEADLINE_SIZE: 28, numeralSize: 36, labelSize: 17, showSubdeck: true };
...
await sharp(`${OUTDIR}/school-bus-seat-belts-ig-portrait-1080x1350.png`).jpeg({ quality: 90 }).toFile(`${igOut}/ig-school-bus-seat-belts-chart-2026-05.jpg`);
await sharp(`${OUTDIR}/school-bus-seat-belts-fb-feed-1200x630.png`).png({ compressionLevel: 9 }).toFile(`${fbOut}/fb-school-bus-seat-belts-chart-2026-05.png`);
console.log(`IG: ${igOut}/ig-school-bus-seat-belts-chart-2026-05.jpg`);
console.log(`FB: ${fbOut}/fb-school-bus-seat-belts-chart-2026-05.png`);

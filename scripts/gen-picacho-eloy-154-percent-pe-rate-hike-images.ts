...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from ambient sunset light. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no faces, no hands, no people visible.`;

const PROMPT = `Wide-angle documentary photograph of a small concrete municipal wastewater treatment lift station structure at golden hour in the Arizona Sonoran desert near a 55-plus retirement community. The squat industrial enclosure has corrugated metal walls with a rusted steel ladder leading to a small flat roof, surrounded by chain-link fencing. Beyond the fence, a manicured retirement community fairway stretches into the distance with palm trees and a low Spanish-style stucco clubhouse silhouette in the background. The desert ground is dust and gravel with a few low desert plants. The sky is a deep desert dusk with low warm light raking across the corrugated metal. Atmosphere: an aging utility serving a community that didn't pick its owner. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-picacho-eloy-154-percent-pe-rate-hike.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'picacho-eloy-154-percent-pe-rate-hike';
const HEADLINE = '154% Sewer Hike. Dutch PE Owner.';
...
]).png({ compressionLevel: 9 }).toFile(OG_OUT);
console.log(`  ${OG_OUT}`);

...
const PROMPT = `Wide-angle documentary photograph of a vacant industrial warehouse exterior at dusk in a Phoenix-area suburban industrial district. A long beige concrete tilt-up wall stretches into the distance with a chain-link fence and an empty parking lot in the foreground. Across an empty arterial street, a second industrial chemical warehouse sits behind a tall security fence with hazard signage silhouetted against the warm sunset glow, but no readable text. A water tower in the far distance. The sky is a deep desert dusk with low warm light raking across the asphalt. Atmosphere: civic decision-making about who lives and works where, made before anyone in the neighborhood knew. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-arizona-mayes-ice-surprise-detention-lawsuit.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'arizona-mayes-ice-surprise-detention-lawsuit';
const HEADLINE = 'Detention Plan. Chlorine Across the Street. AG Sues.';

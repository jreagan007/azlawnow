...
const PROMPT = `Wide-angle documentary photograph of a school's mechanical room HVAC air handler, side panel removed, exposing a dusty pleated air filter half-mounted in a metal frame with visible gaps along the edges where the filter doesn't seat flush against the housing. Tangled ducts, exposed sheet metal, fluorescent shop light from above, warm amber dust particles suspended in a backlit beam of sunlight from a small high window. Industrial atmosphere, the workings behind the wellness brochure. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-arizona-schools-merv-13-filter-bypass.webp';
...
const HEADLINE = 'Hospital-Grade Filter. Unsealed Housing. Air Bypasses.';

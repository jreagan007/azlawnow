...
const PROMPT = `Wide-angle documentary photograph of a contemporary suburban Tempe Arizona charter school building exterior at golden hour. A clean modular architecture with sand-colored stucco, large modern windows, a metal canopy entrance walkway, and a paved circular drop-off loop in the foreground. Empty parking lot stretches into the distance with palm trees lining the perimeter. The desert light is warm and slanted, casting long shadows across the asphalt. A leasing-realty real estate sign post stands isolated in front of an adjacent vacant building lot. Atmosphere: institutional permanence built from public dollars, sitting on land owned by someone else. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-asu-prep-related-party-lease.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'asu-prep-related-party-lease';
const HEADLINE = '$5.4M Loss. $11.5M Related-Party Lease.';

/**
 * Hero photo + OG card for bnsf-phoenix-subdivision-corpus.
 * Imagen-4 generates a documentary photo of a long west-Phoenix arterial
 * corridor with multiple grade crossings receding into the distance, then
 * Sharp composites the brand-locked OG card lockup on top.
 */
...
const PROMPT = `Wide-angle documentary photograph looking down a long straight west Phoenix arterial avenue at twilight, single set of railroad crossing arms in the foreground (lowered position, red lights), and the hint of a second set of crossing arms a few blocks further down the road. The road extends toward palm-tree silhouettes, low 1970s strip-mall storefronts, and a faint downtown Phoenix skyline silhouette. Empty asphalt, painted yellow center stripe, weathered shoulder. Desert sky transitioning from warm gold at the horizon to deep navy overhead. Atmosphere: a corridor where the same engineering choices repeat for miles. ${STYLE}`;
...
const HERO_OUT = '/Users/taqticlaw/Projects/azlawnow/public/images/heroes/dj-bnsf-phoenix-subdivision-corpus.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  hero: ${HERO_OUT}`);

const SLUG = 'bnsf-phoenix-subdivision-corpus';
const HEADLINE = '5 BNSF Filings. 3 Crossings. Same Phrase.';

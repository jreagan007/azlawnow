...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from ambient light sources only. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no people faces, no hands, no children visible, no minors, no school logos.`;

const PROMPT = `Wide-angle documentary photograph of an empty late-afternoon school district administration office or hearing room interior. A long varnished oak conference table, ten empty leather-bound chairs, a single brass desk lamp lit on a side credenza throwing warm amber across a stack of file folders, a tall wooden bookshelf in the background half in shadow holding bound volumes, soft natural light through tall vertical blinds. The atmosphere is civic accountability after the room has emptied, the place where decisions are made about who teaches and who doesn't. Documentary editorial restraint. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-arizona-325-educator-discipline-2024.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'arizona-325-educator-discipline-2024';
const HEADLINE = '325 Disciplined. 36% Sexual Misconduct. Maricopa Drove It.';

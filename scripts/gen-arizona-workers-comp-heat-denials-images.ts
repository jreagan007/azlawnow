...
const STYLE = `Shot on Kodak Portra 400 film, shallow depth of field, cinematic composition, slight film grain. Color graded: deep navy blue in shadows, warm sienna in midtones, warm gold accent from harsh sun. High contrast, slightly desaturated, editorial magazine quality, documentary aesthetic. No text, no words, no letters, no numbers, no logos, no watermarks, no faces, no hands, no people visible.`;

const PROMPT = `Wide-angle documentary photograph of an empty industrial warehouse loading dock in Phoenix Arizona at midday, 110-degree heat haze visible in the distance shimmering off the asphalt. A forklift sits abandoned mid-shift on a concrete loading bay with a half-pallet of bottled water beside it. Heavy yellow safety bollards. A single hardhat rests on the corner of the dock. The sun is directly overhead casting nearly no shadows. The American flag hangs limp against a metal building. Atmosphere: a workplace where the heat became too dangerous to continue but the framework to make that decision protected didn't exist. ${STYLE}`;
...
const HERO_OUT = 'public/images/heroes/dj-arizona-workers-comp-heat-denials.webp';
await sharp(buf).resize(1200, 675, { fit: 'cover' }).webp({ quality: 88 }).toFile(HERO_OUT);
console.log(`  ${HERO_OUT}`);

const SLUG = 'arizona-workers-comp-heat-denials';
const HEADLINE = 'AZ Heat Workers Comp: Denial Rate Isn\'t Public';
